...
// TABLA OFICIAL: PRECIO NORMAL (LISTA) Y PRECIO CON DESCUENTO (BASE)
export const OFFICIAL_KIT_PRICES = {
  K1:  { lista: 16106100, desc: 14510000, mult: 1.110 },
  K2:  { lista: 19024360, desc: 17170000, mult: 1.108 },
  K3:  { lista: 24155040, desc: 21840000, mult: 1.106 },
  K4:  { lista: 27070080, desc: 24520000, mult: 1.104 },
  K5:  { lista: 27715300, desc: 25150000, mult: 1.102 },
  K6:  { lista: 31452500, desc: 28750000, mult: 1.094 },
  K7:  { lista: 52201680, desc: 47760000, mult: 1.093 },
  K8:  { lista: 54490800, desc: 49900000, mult: 1.092 },
  K9:  { lista: 62285190, desc: 57090000, mult: 1.091 },
  K10: { lista: 77379100, desc: 70990000, mult: 1.090 }
};
...
export function calcKitPricing(kit) {
  ...
  if (data) {
    return {
      bom,
      precioFinal: data.lista,
      precioConDescuento: data.desc
    };
  }
  ...
  const precioConDescuento = Math.round(precioFinal * factor);
  return { bom, precioConDescuento, precioFinal };
}

// SELECCIÓN DE KIT RECOMENDADO POR INVERSOR
export function recommendKit(neededWp, neededBankKwh, neededInverterW) {
  const candidato = KITS.find(k => k.inversorW >= neededInverterW);
  if (candidato) {
    const cumple = candidato.totalWp >= (neededWp || 0) && candidato.totalBateriaKwh >= (neededBankKwh || 0);
    return { kit: candidato, cumple };
  }
...
export function calcManualBattery(bankKwh, voltage, battKwh, requiredInverterW = 0) {
  ...
  if (voltage === 24) {
    ...
    // En 24V limitamos a 1 pareja (máx 2 baterías) en cálculo automático para evitar desbalance
...
// ASIGNACIÓN DE DESCUENTO POR INVERSOR CON DESEMPATE POR PANELES
export function getClosestKitDiscount(inverterW, numPaneles = 0) {
  ...
  if (w <= 3000) {
    return p <= 4 ? { kitId: 'K1', factor: 0.900, pct: 10.0 } : { kitId: 'K2', factor: 0.910, pct: 9.0 };
  }
  if (w <= 5000) {
    return p <= 6 ? { kitId: 'K3', factor: 0.920, pct: 8.0 } : { kitId: 'K4', factor: 0.920, pct: 8.0 };
  }
  if (w <= 6400) {
    return p <= 9 ? { kitId: 'K5', factor: 0.925, pct: 7.5 } : { kitId: 'K6', factor: 0.930, pct: 7.0 };
  }
  if (w <= 8000) {
    return { kitId: 'K7', factor: 0.940, pct: 6.0 };
  }
  if (w <= 10000) {
    return { kitId: 'K8', factor: 0.945, pct: 5.5 };
  }
  if (w <= 12000) {
    return { kitId: 'K9', factor: 0.950, pct: 5.0 };
  }
  return { kitId: 'K10', factor: 0.954, pct: 4.6 };
}
...
export function calcOptimizedPrice(bom, solution) {
  ...
  const precioConDescuento = Math.round(precioFinal * discountInfo.factor);
  return {
    precioFinal,
    precioConDescuento,
    ahorro: precioFinal - precioConDescuento,
    descuentoPct: discountInfo.pct,
...
export function calcCustomKitProposal(config) {
  ...
  return {
    ...
    precioFinal,
    precioConDescuento,
    discountFactor: discountInfo.factor,
    descuentoPct: discountInfo.pct,
    kitEquivalente: discountInfo.kitId
  };
}
